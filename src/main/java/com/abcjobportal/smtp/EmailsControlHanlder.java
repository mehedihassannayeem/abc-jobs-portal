...
public class EmailsControlHanlder {

	/*
	 * in real life a different SMTP server and host will be there for sending
	...
	private final String smtpLogPassword = "kbdgawlhrndltwcf";
	...
	//
	//
	// send email function for registration
	public boolean regEmailSend(NewRegUser user, String logo) {
		boolean f = false;

		final String emailSubject = user.getRegUserName() + " - confirm your registration on portal.";
		final String emailBody = "Hi! " + user.getRegUserName()
				+ ".\rThank you for your registration.\r\rPlease use the OTP to confirm your registration in the portal."
				+ "\rYour OTP is - " + user.getRegUserOTP() + "\r\rThis is an auto-generated email to "
				+ user.getRegUserEmail() + " please do not reply or respond to this email."
				+ "\r\rPlease ignore this email if you did not register on \"ABC Jobs Portal\"."
				+ "\rIf you believe someone else has used your email to register, please notify us by filing a complaint."
		...
		try {
			...
			mimeMessage.addRecipient(Message.RecipientType.TO, new InternetAddress(user.getRegUserEmail()));
